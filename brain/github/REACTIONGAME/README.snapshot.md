<!-- generatedBy: brain:sync-github; source: https://github.com/uset82/REACTIONGAME/blob/main/README.md; checkedOn: 2026-07-31; redactions: 0 -->
...
- Player 1: 1 button + 1 LED (with current?limit resistor) + 1 pull?up resistor (10 k?)
- Player 2: 1 button + 1 LED (with current?limit resistor) + 1 pull?up resistor (10 k?)
- Reflex LED: 1 LED + 1 current?limit resistor

Buttons are wired active?low (external 10 k? pull?up to 3V3). Pressing a button pulls the GPIO input to GND.
...
- Inputs are configured with no internal pulls. Your buttons should provide external 10 k? pull?ups (idle HIGH). If you must use internal pulls instead, change PB8/PB9 to `GPIO_PULLUP` in `MX_GPIO_Init` and keep active?low logic.
