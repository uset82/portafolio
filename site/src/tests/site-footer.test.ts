...
test("footer closes with one honest contact path and one verified public profile", () => {
  ...
  const secondaryNavigation = siteContent.secondaryNavigation.map(({ href, label }) => ({
    href,
    label,
  }));
  const markup = renderToStaticMarkup(
    createElement(SiteFooter, {
      content: siteContent.metadata.footer,
      navigation,
      secondaryNavigation,
    }),
  );
  ...
  // The invitation moved into the About section, where a visitor is deciding.
  // The footer must not restate it, or the page closes twice.
  assert.doesNotMatch(markup, /Public contact method pending/);
  assert.doesNotMatch(markup, /The contact route remains privacy-first/);
  assert.doesNotMatch(markup, /site-footer__signal/);
  ...
  // Routes are grouped, not run together as one numbered list.
  assert.match(markup, /aria-label="Secondary footer navigation"/);
  assert.match(markup, /Start here/);
  assert.match(markup, /Also here/);
...
test("footer styling preserves focus feedback, touch sizes, mobile order, and reduced motion", () => {
  ...
  assert.match(styles, /\.site-footer__routes nav a[^}]*min-height:\s*2\.5rem/);
