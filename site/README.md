...
`src/lib/three/scene-state.ts` is the single application-state owner for the optional live scene. Its version 3 external store controls quality, system/explicit motion preference, pause/visibility, mute-first sound, lifecycle/progress/errors, selected artifact, and cancellable camera requests; React observes snapshots but does not update state from the frame loop. `ObservatorySceneShell` projects that state into one perspective camera, a locked-palette local light rig, and seven named groups covering all 12 registry assets. The environment has no external HDR or asset URL. The shell remains behind the poster-first critical-asset gate, so the rights-pending robot currently keeps the production homepage on its complete semantic poster.

The integrated water-group proof of concept uses one procedural ripple/reflected-light shader with 6,912 triangles and at most 30 demand invalidations per second; it allocates no textures, reflection targets, shadows, or post passes. Reduced motion/quality uses a still two-triangle standard material, while static or not-ready state renders no water so the existing poster remains visible. These are deterministic structural limits; live production renderer measurements remain gated by the rights-cleared critical model.

`ObservatorySceneRuntimeProvider` owns browser capability assessment outside the optional Canvas. WebGL2 support, viewport, DPR, optional device memory/CPU/network/battery hints, reduced motion/data, visibility, and Auto/Full/Reduced/Poster preference resolve through `capability-policy.ts` into versioned scene state. Unsupported WebGL2 always preserves the poster; missing optional APIs are neutral; every listener is cleaned up. The mounted `ObservatoryExperienceControls` disclosure adds system/explicit reduced motion, Pause/Resume, a truthful unavailable Sound status, strict local preference persistence, and atomic Reset. Desktop keeps the drawer above the scene and CC AI launcher; mobile places it in normal flow after the poster, with 44px controls and no content clipping.
...
| `OPENROUTER_MODEL`                      | Server only               | Optional in prototype mode         | Overrides the `openrouter/free` prototype default with a named OpenRouter slug such as `z-ai/glm-5.3-flash`         |
| `OPENROUTER_FALLBACK_MODELS`            | Server only               | Optional in prototype mode         | Supplies up to four ordered, comma-separated fallback model IDs (example: `openrouter/free`)                        |
| `CC_AI_REASONING_EFFORT`                | Server only               | Optional; defaults to `low`        | Sets reasoning effort (`none`–`max`) for thinking models; high values can consume the whole answer token budget     |
...
`POST /api/cc-ai` is currently an intentionally non-streaming JSON boundary. It validates a bounded message/history payload, issues a server request ID, propagates disconnect/timeout aborts, caps provider tokens and returned characters, discloses the primary, fallback, selection type, variability, and actual responding model IDs, sets `Cache-Control: no-store`, and maps provider failures to safe public errors. Visitors still receive only the normalized error; the server additionally logs the failure cause (normalized code, HTTP status, truncated SDK message, finish reason, requested model IDs) so a retired model slug is distinguishable from a revoked key. Message bodies, history, and credentials are never logged.

Prototype mode defaults to the variable `openrouter/free` router, supports a named-model override, and sends configured fallback models in order. Reasoning models are requested at `CC_AI_REASONING_EFFORT` (default `low`) inside a 180 s prototype budget: reasoning tokens are drawn from the same completion allowance as the answer, so a higher effort can exhaust it and return `finish_reason: "length"` with no prose. It denies providers that collect prompts for training but does not require zero-data-retention endpoints, because that stricter filter can leave the free router with no eligible endpoint. Production mode fails closed unless an explicit paid named primary model is configured, uses a separate paid fallback list, rejects free routes, and requires both denied provider data collection and zero data retention. Provider fallback stays inside the active mode's constraints. If no compliant provider can serve the request, the route returns a safe unavailable state instead of weakening the policy. Payment exhaustion, rate limiting, and provider failure are normalized into distinct 402/429/unavailable service states for the later chat UI.
